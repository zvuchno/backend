...
def create_catalog_search(apps, schema_editor):
    """Создаёт materialized view глобального поиска каталога."""
...
def drop_catalog_search(apps, schema_editor):
    """Удаляет materialized view глобального поиска."""
    if schema_editor.connection.vendor != 'postgresql':
        return
...
SELECT
    'merch_kind' AS entity_type,
    merch_kind.id AS entity_id,
    merch_kind.name AS name,
    NULL::varchar AS artist_name,
    NULL::varchar AS genre_name,
    NULL::varchar AS merch_kind_name,
    NULL::text AS variant_values,
    ...
    merch_kind.name AS search_text,
...
FROM store_merchkind merch_kind

WHERE
    merch_kind.is_active = TRUE
...
DROP_SQL = """
DROP MATERIALIZED VIEW IF EXISTS catalog_search;
"""
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            create_catalog_search,
            reverse_code=drop_catalog_search,
        ),
    ]
