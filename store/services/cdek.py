...
class CDEKService:
    ...
    def _calculate_for_artist(
        ...
        if delivery_type == 'pickpoint':
            tariff_code = self.tariff_code_pickpoint
        elif delivery_type == 'courier':
            tariff_code = self.tariff_code_courier
        else:
            raise ValidationError({
        ...
        try:
            ...
            response.raise_for_status()
            data = response.json()
            logger.info(f'data: {data}')  # Отладочный  !!

            # Возвращаем Decimal для сохранения точности при сложении денег
            return Decimal(str(data.get('total_sum', '0.00')))

        except requests.RequestException as e:
            logger.error(
                'Не удалось выполнить расчет стоимости доставки '
