...
def test_merch_detail_has_expected_contract(
    ...
    response = api_client.get(catalog_merch_detail_url(product.merch))
...
def test_catalog_release_detail_contains_album_and_carrier_variants(
    ...
    cd_kind = MerchKindFactory(name='CD', slug='cd', is_carrier=True)
    vinyl_kind = MerchKindFactory(name='Винил', slug='vinyl', is_carrier=True)
    ...
    response = api_client.get(catalog_release_detail_url(album_product.album))

    assert response.status_code == status.HTTP_200_OK
    assert_catalog_release_detail_contract(response.data)

    variant_ids = {item['variant_id'] for item in response.data['variants']}
