...
become_artist_schema = extend_schema(
    ...
    description=(
        'Создает профиль артиста или лейбла для текущего '
        'авторизованного пользователя. Тип профиля определяется '
        'полем profile_type. Если такой профиль уже существует, '
        'возвращает ошибку.'
    ),
)
